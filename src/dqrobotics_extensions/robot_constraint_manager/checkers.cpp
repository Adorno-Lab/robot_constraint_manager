                                         ...
                                         const Checkers::MODE& mode)
{
    ...
    if (n_A != n_B)
    {
        if (mode == Checkers::MODE::PANIC )
            throw std::runtime_error(std::string("Panic with DQ_robotics::Checkers::check_column_matrix_sizes(A, B). ")
...
bool Checkers::check_constraint_sizes(const MatrixXd &A, const VectorXd &b, const double &optimization_vector_size, const MODE &mode)
{
    ...
    {
        if (mode == Checkers::MODE::PANIC )
            throw std::runtime_error(std::string("Panic with DQ_robotics::Checkers::check_constraint_sizes(A, b, dim). ")
